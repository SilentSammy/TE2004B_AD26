...
// Diagnostic test: the laptop joins this Pico W access point.
const char *AP_SSID = "PicoLatency";
const char *AP_PASSWORD = "pico-test-2026";
const IPAddress AP_ADDRESS(192, 168, 4, 1);
...
void announceTo(IPAddress address, uint16_t port) {
  if (udp.beginPacket(address, port)) {
    udp.print("HELLO ");
    udp.print(WiFi.softAPmacAddress());
    udp.endPacket();
  }
}
...
void setup() {
  ...
  WiFi.mode(WIFI_AP);
  WiFi.softAPConfig(AP_ADDRESS, AP_ADDRESS, AP_SUBNET);
  ...
  announceTo(IPAddress(192, 168, 4, 255), REGISTRATION_PORT);
}
